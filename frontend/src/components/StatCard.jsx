export default function StatCard({ label, value, delta, tone = 'green' }) {
  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-4 sm:p-5 shadow-card hover:shadow-card-hover hover:border-border-strong transition-all">
      <div className="text-[10px] uppercase tracking-[0.2em] text-muted font-medium">{label}</div>
      <div className={`font-display text-2xl sm:text-3xl font-bold mt-2 ${toneClass}`}>{value}</div>
